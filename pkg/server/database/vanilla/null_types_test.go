...
import (
	...
	"github.com/claion-org/claiflow/pkg/server/database/vanilla"
)
...
func TestNullJson(t *testing.T) {
	{
		{
			obj := vanilla.NewNullJson([]string{"foo", "bar"})
			expected := `["foo","bar"]`
		...
		{
			obj := vanilla.NewNullJson(map[string]interface{}{
				"string": "foo",
				"number": 987654321,
				"bool":   true,
				"nil":    nil,
			...
			if err := JsonUnmarshal(expected, obj, func(a interface{}) error {
				v, ok := a.(*vanilla.NullJson[map[string]interface{}])
				if !ok {
					return fmt.Errorf("diff type")
				}
